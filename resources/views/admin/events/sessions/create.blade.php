...
@section('title', 'Add Session - TREC')
...
<div class="max-w-3xl">
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        <form action="{{ route('admin.events.sessions.store', $event) }}" method="POST" class="divide-y divide-slate-200">
            @csrf
            ...
            <div class="p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required class="w-full rounded-lg border-slate-300 focus:border-red-500 focus:ring-red-500">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="category" id="category" class="w-full rounded-lg border-slate-300 focus:border-red-500 focus:ring-red-500">
                            <option value="keynote" {{ old('category') == 'keynote' ? 'selected' : '' }}>Keynote</option>
                            ...
                            <option value="break" {{ old('category') == 'break' ? 'selected' : '' }}>Break / Lunch</option>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3" class="w-full rounded-lg border-slate-300 focus:border-red-500 focus:ring-red-500">{{ old('description') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="p-8 space-y-6 bg-slate-50">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="date" name="session_date" id="session_date" value="{{ old('session_date', $event->event_date?->format('Y-m-d')) }}" class="w-full rounded-lg border-slate-300 focus:border-red-500 focus:ring-red-500">
                    </div>
                    <div>
                        <label for="start_time" class="block text-sm font-medium text-slate-700 mb-1">Start Time</label>
                        <input type="time" name="start_time" id="start_time" value="{{ old('start_time') }}" class="w-full rounded-lg border-slate-300 focus:border-red-500 focus:ring-red-500">
                    </div>
                    <div>
                        <label for="end_time" class="block text-sm font-medium text-slate-700 mb-1">End Time</label>
                        <input type="time" name="end_time" id="end_time" value="{{ old('end_time') }}" class="w-full rounded-lg border-slate-300 focus:border-red-500 focus:ring-red-500">
                    </div>

                    <div class="md:col-span-2">
                        <label for="venue_room" class="block text-sm font-medium text-slate-700 mb-1">Room / Location</label>
                        <input type="text" name="venue_room" id="venue_room" value="{{ old('venue_room') }}" class="w-full rounded-lg border-slate-300 focus:border-red-500 focus:ring-red-500" placeholder="e.g. Main Hall">
                    </div>
