...
<div style="background:#f8fafc;color:#1e293b;min-height:100vh;padding:3rem 2rem">
  <div class="wrap" style="max-width:900px">
    <a href="{{ route('admin.blog') }}" style="color:#64748b;text-decoration:none;font-size:14px;margin-bottom:1rem;display:inline-flex;align-items:center;gap:.5rem;transition:color .2s">
    ...
    <h1 style="font-family:var(--font-h);font-size:2rem;font-weight:900;margin-bottom:2rem;color:#1e293b;display:flex;align-items:center;gap:.75rem">
      <svg class="lucide lucide-edit-3" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>
      Edit Insight
    </h1>
    
    <form method="POST" action="{{ route('admin.blog.update', $post) }}" enctype="multipart/form-data" style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:2rem;box-shadow:0 1px 3px rgba(0,0,0,.05)">
      @csrf
      @method('PUT')
      ...
      <div style="margin-bottom:2rem">
        ...
        <input type="text" name="title" required style="width:100%;padding:1rem;background:#f8fafc;border:1px solid #cbd5e1;border-radius:8px;color:#1e293b;font-family:inherit;font-size:1rem;transition:all .2s" placeholder="Enter insight title" value="{{ old('title', $post->title) }}">
      ...
      <div style="margin-bottom:2rem">
        ...
        <input type="text" name="slug" required style="width:100%;padding:1rem;background:#f8fafc;border:1px solid #cbd5e1;border-radius:8px;color:#1e293b;font-family:inherit;font-size:1rem;transition:all .2s" placeholder="url-slug-format" value="{{ old('slug', $post->slug) }}">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:2rem">
        <div>
          ...
          <input type="text" name="category" required style="width:100%;padding:1rem;background:#f8fafc;border:1px solid #cbd5e1;border-radius:8px;color:#1e293b;font-family:inherit;font-size:1rem;transition:all .2s" placeholder="e.g., Mental Health" value="{{ old('category', $post->category) }}">
        ...
        <div>
          ...
          <input type="number" name="read_time" min="1" style="width:100%;padding:1rem;background:#f8fafc;border:1px solid #cbd5e1;border-radius:8px;color:#1e293b;font-family:inherit;font-size:1rem;transition:all .2s" placeholder="5" value="{{ old('read_time', $post->read_time) }}">
      ...
      <div style="margin-bottom:2rem">
        ...
        @if($post->image_url)
          <div style="margin-bottom:1rem;background:#f8fafc;padding:1rem;border-radius:8px;display:flex;align-items:center;justify-content:space-between;border:1px solid #e2e8f0">
            <span style="font-size:14px;color:#475569">Current image: {{ basename($post->image_url) }}</span>
            <span style="font-size:12px;color:#94a3b8">Upload new to replace</span>
          </div>
        @endif
      ...
      <div style="margin-bottom:2rem">
        ...
        <textarea name="excerpt" style="width:100%;padding:1rem;background:#f8fafc;border:1px solid #cbd5e1;border-radius:8px;color:#1e293b;font-family:inherit;font-size:1rem;resize:vertical;min-height:100px;transition:all .2s" placeholder="Brief summary of the insight">{{ old('excerpt', $post->excerpt) }}</textarea>
      ...
      <div style="margin-bottom:2rem">
        <label style="display:flex;align-items:center;gap:.5rem;font-weight:600;font-size:14px;color:#1e293b">
          ...
          <span>Publish Date & Time (leave empty to keep as draft)</span>
        </label>
        <input type="datetime-local" name="published_at" style="margin-top:.75rem;padding:.75rem;background:#f8fafc;border:1px solid #cbd5e1;border-radius:8px;color:#1e293b;font-family:inherit;transition:all .2s;width:100%" value="{{ old('published_at', $post->published_at ? $post->published_at->format('Y-m-d\TH:i') : '') }}">
